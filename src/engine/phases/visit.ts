import type { ConversationState, Extracted, OfferedProject, TurnGoal, VisitState, AnswerTopic } from '../types.js';
import { hasExplicitTime, parseVisitSlot, reparseVisitTime } from '../visit-slot.js';
...
const VISIT_DEFERRABLE_TOPICS: AnswerTopic[] = [
  'emi',
  'legal',
  'price',
  'media',
...
function step(input: {
  ...
  if (input.named.length > 1) {
    const [first, ...rest] = input.named;
    projectId = first!.projectId;
    projectName = first!.name;
    queued = rest.map((p) => ({ projectId: p.projectId, projectName: p.name }));
  } else if (input.named.length === 1) {
    projectId = input.named[0]!.projectId;
    projectName = input.named[0]!.name;
  ...
  if (!projectId || !projectName) {
    ...
    return {
      ...
      ask: 'project',
      copy,
      state: { ...input.prior, askCount: (input.prior.askCount ?? 0) + 1, lastAsk: 'project' },
    };
  }
  ...
  if (!slot && !input.prior.slotText) {
    if (BARE_AFFIRM.test(input.text.trim())) {
      return {
        kind: 'visit_ask',
        ask: 'day',
        copy: `Which *day* works for *${projectName}*? — for example Saturday or tomorrow.`,
        state: {
          projectId,
          projectName,
          queued,
          askCount: (input.prior.askCount ?? 0) + 1,
          lastAsk: 'day',
        },
      };
    }
    const copy = declined
      ? `No problem — which day works for *${projectName}*?`
      : `Which day works for your visit to *${projectName}*? (e.g. Saturday, tomorrow)`;
    return {
      kind: 'visit_ask',
      ask: 'day',
      copy,
      state: { projectId, projectName, queued, askCount: (input.prior.askCount ?? 0) + 1, lastAsk: 'day' },
    };
  }
  ...
  const copy = `Shall I block *${parsed.humanLabel}* for your visit to *${projectName}*? Reply yes to confirm.`;
  return {
    kind: 'visit_propose',
