...
/** Narrow “process next step” — not “next payment” / facet asks. */
export const ASK_NEXT_STEP_RE =
  /\b(?:what(?:'s| is)?\s+(?:the\s+)?next(?:\s+step)?|what\s+should\s+(?:i|we)\s+do(?:\s+next)?|what\s+do\s+(?:i|we)\s+do(?:\s+(?:now|next))?|where\s+do\s+we\s+go\s+from\s+here|how\s+do\s+(?:i|we)\s+proceed(?:\s+from\s+here)?|how\s+do\s+we\s+move\s+forward|ok(?:ay)?\s+what\s+now|what\s+happens\s+next|what'?s\s+my\s+next\s+move|guide\s+me\s+on\s+the\s+next\s+step|take\s+me\s+to\s+the\s+next\s+step|aage\s+kya(?:\s+karna\s+hai)?|ab\s+kya\s+karu|next\s+step\s+kya\s+hai)\b/i;
