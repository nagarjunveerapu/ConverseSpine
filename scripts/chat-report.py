...
Drive a conversation against a live deployment and publish what happened.
    ...
    python3 scripts/chat-report.py scripts/scenarios/budget-fit.json \
    ...
    for t in scn['turns']:
        ...
        turns.append({
            **t,
            ...
            # The ledger is keyed on the Desk thread, not the advisor
            # session. Take the id the response hands back rather than
            # guessing a join — the first attempt guessed and silently
            # attached nothing.
            'nd': resp.get('nd_thread_id') or resp.get('thread_id'),
            'timings': (resp.get('debug') or {}).get('timings') or {},
        })
    return turns
...
def attach_shadow(turns, db, env):
    ...
    if not nd:
        print('  (no nd_thread_id on any turn — no shadow to attach)')
        return turns
    rows = d1(db, env, f"""
        SELECT json_extract(action_plan_json,'$.chip_shadow') sh
        FROM turn_ledger WHERE thread_id = '{nd}' ORDER BY created_at""")
...
def render(scn, turns, meta, essay_html=''):
    ...
    # Embedding is the only stage in the turn that costs real money in time:
    # the whole regex ladder measures under a millisecond, one Workers AI call
    # measures ~266ms. Priced per call, so the count is the number to watch —
    # a turn that embeds four times paid four fixed overheads for one question.
    embed_calls = sum((t.get('timings') or {}).get('embed_calls', 0) for t in turns)
    embed_ms = sum((t.get('timings') or {}).get('embed_ms', 0) for t in turns)
    worst_calls = max(((t.get('timings') or {}).get('embed_calls', 0) for t in turns), default=0)
    ...
    parts.append('<section class="strip">'
        ...
        + (f'<div class="stat {"bad" if worst_calls > 1 else "good"}">'
           f'<div class="k">Embed calls</div>'
           f'<div class="v">{embed_calls}<span style="font-size:13px;opacity:.6">'
           f' · {embed_ms}ms</span></div></div>' if embed_calls else '')
        + '</section>')
    ...
    for i, t in enumerate(turns, 1):
        ...
        tm = t.get('timings') or {}
        # One embed a turn is the target; the tag goes red above that so a
        # regression is visible in the transcript without opening the numbers.
        embed_tag = (f'<span class="tag{" slow" if tm["embed_calls"] > 1 else ""}">'
                     f'{tm["embed_calls"]}× embed {tm.get("embed_ms", 0)}ms</span>'
                     if tm.get('embed_calls') else '')
        parts.append(f'''<article class="turn">
          <div class="turn-head"><span class="turn-n">{i:02d}</span>
            ...
            <span class="tag{slow_cls}">{t["secs"]:.2f}s</span>{embed_tag}</div>
    ...
    parts.append(f'<footer>Generated by scripts/chat-report.py against '
...
def main():
    ...
    ap.add_argument('--out', default='chat-report.html')
    a = ap.parse_args()
